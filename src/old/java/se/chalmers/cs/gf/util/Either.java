...
public class Either<A,B> {
        ...
        public A getLeft() {
                if (!isLeft)
                        throw new NoSuchElementException("Called getLeft() when there is only a right.");
                return left;
        }

        public B getRight() {
                if (isLeft)
                        throw new NoSuchElementException("Called getRight() when there is only a left.");
                return right;
        }

        public static <A,B> Either<A,B> mkLeft (A x) {
                return new Either<A,B>(x, null, true);
        }

        public static <A,B> Either<A,B> mkRight (B x) {
                return new Either<A,B>(null, x, false);
        }
